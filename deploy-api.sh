...
START_CMD="pip install --no-cache-dir --upgrade 'torch>=2.6.0' --index-url https://download.pytorch.org/whl/cu124 && pip install --no-cache-dir 'FlagEmbedding>=1.3.0' runpod==1.7.6 && curl -fsSL '${HANDLER_URL}' -o /handler.py && python -u /handler.py"
